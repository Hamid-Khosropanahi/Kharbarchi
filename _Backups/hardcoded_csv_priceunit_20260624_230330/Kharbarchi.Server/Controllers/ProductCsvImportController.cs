...
ORDER BY Id;";
        ...
        return Ok(new
        {
            ...
            pricingRule = "CSV unit prices are read as kg price. If priceUnit=rial: kg price = input / 10. If toman: kg price = input. Bulk = kg price * kg. Retail = packs per carton * ((kg price * unit kg) + package fee per pack)."
        });
    }
...
ON DUPLICATE KEY UPDATE QueueStatus = 'pending', SKU = VALUES(SKU), ProductSlug = VALUES(ProductSlug), WooPayloadJson = VALUES(WooPayloadJson), UpdatedAtUtc = UTC_TIMESTAMP(6);",
    ...
    private static async Task ResetGeneratedProductTablesAsync(MySqlConnection connection, CancellationToken cancellationToken)
    {
        ...
        await ExecuteAsync(connection, "TRUNCATE TABLE KHB_Product_Update_Queue;", cancellationToken);
        await ExecuteAsync(connection, "TRUNCATE TABLE KHB_Product_Final;", cancellationToken);
        await ExecuteAsync(connection, "TRUNCATE TABLE KHB_Package_Type;", cancellationToken);
        await ExecuteAsync(connection, "TRUNCATE TABLE KHB_Commodity;", cancellationToken);
        await ExecuteAsync(connection, "TRUNCATE TABLE KHB_Category_Map;", cancellationToken);
        await ExecuteAsync(connection, "TRUNCATE TABLE KHB_Source_Product;", cancellationToken);
        await ExecuteAsync(connection, "TRUNCATE TABLE khb_sale_products;", cancellationToken);
        await ExecuteAsync(connection, "TRUNCATE TABLE khb_product_main_groups;", cancellationToken);
