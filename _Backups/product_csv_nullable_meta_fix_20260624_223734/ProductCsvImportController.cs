...
ORDER BY Id;";
        ...
        foreach (var row in rows)
        {
            ...
            var mainSlug = BuildEnglishSlug(FirstNotEmpty(row.ProductEnglishName, row.CategorySlug, row.MainProductSlug, row.Sku, mainName));
            var categoryName = FirstNotEmpty(row.CategoryName, mainName);
            var categorySlug = BuildEnglishSlug(FirstNotEmpty(row.CategorySlug, row.ProductEnglishName, row.BrandEnglishName, categoryName));
...
LIMIT @Take OFFSET @Skip;";
        ...
        string sourceSuffix)
    {
        ...
        var slugBase = BuildEnglishSlug(FirstNotEmpty(row.ProductEnglishName, row.ProductSlug, row.Sku, productName));
        ...
        return new SaleProductDraft(
            ...
            ProductSlug: BuildEnglishSlug(slugBase + "-" + packageCode),
            Sku: sku,
...
ON DUPLICATE KEY UPDATE PackageGroup = VALUES(PackageGroup), PackageCode = VALUES(PackageCode), PackageTitle = VALUES(PackageTitle), UnitWeightKg = VALUES(UnitWeightKg), PacksPerCarton = VALUES(PacksPerCarton), PackagingPricePerPack = VALUES(PackagingPricePerPack), UpdatedAtUtc = UTC_TIMESTAMP(6);",
        ...
        var payload = JsonSerializer.Serialize(new
        {
            ...
            meta_data = new[]
            {
                ...
                new { key = "_kharbarchi_package_group", value = product.PackagingGroup },
                new { key = "_kharbarchi_package_code", value = product.PackageCode },
                new { key = "_kharbarchi_packs_per_carton", value = product.PacksPerCarton.ToString(CultureInfo.InvariantCulture) },
                new { key = "_kharbarchi_unit_weight_kg", value = product.UnitWeight.ToString("0.####", CultureInfo.InvariantCulture) }
            }
        }, JsonOptions);

        await ExecuteUpsertAsync(connection, @"
INSERT INTO KHB_Product_Final
(SourceKey, MainGroupId, CategorySourceKey, CommoditySourceKey, PackageSourceKey, ProductName, ProductEnglishName, ProductSlug, SKU, SaleCashPrice, SaleCreditPrice, BuyCashPrice, BuyCreditPrice, Status, CatalogVisibility, WooPayloadJson, CreatedAtUtc, UpdatedAtUtc)
VALUES
(@SourceKey, @MainGroupId, @CategorySourceKey, @CommoditySourceKey, @PackageSourceKey, @ProductName, @ProductEnglishName, @ProductSlug, @SKU, @SaleCashPrice, @SaleCreditPrice, @BuyCashPrice, @BuyCreditPrice, @Status, @CatalogVisibility, @WooPayloadJson, UTC_TIMESTAMP(6), UTC_TIMESTAMP(6))
ON DUPLICATE KEY UPDATE ProductName = VALUES(ProductName), ProductEnglishName = VALUES(ProductEnglishName), ProductSlug = VALUES(ProductSlug), SKU = VALUES(SKU), SaleCashPrice = VALUES(SaleCashPrice), SaleCreditPrice = VALUES(SaleCreditPrice), BuyCashPrice = VALUES(BuyCashPrice), BuyCreditPrice = VALUES(BuyCreditPrice), Status = VALUES(Status), CatalogVisibility = VALUES(CatalogVisibility), WooPayloadJson = VALUES(WooPayloadJson), UpdatedAtUtc = UTC_TIMESTAMP(6);",
            cancellationToken,
...
(SourceKey, QueueStatus, ActionType, SKU, ProductSlug, WooPayloadJson, CreatedAtUtc, UpdatedAtUtc)
VALUES
(@SourceKey, 'pending', 'upsert', @SKU, @ProductSlug, @WooPayloadJson, UTC_TIMESTAMP(6), UTC_TIMESTAMP(6))
ON DUPLICATE KEY UPDATE QueueStatus = 'pending', SKU = VALUES(SKU), ProductSlug = VALUES(ProductSlug), WooPayloadJson = VALUES(WooPayloadJson), UpdatedAtUtc = UTC_TIMESTAMP(6);",
            cancellationToken,
            ("@SourceKey", product.SourceRowHash),
            ("@SKU", product.Sku),
            ("@ProductSlug", product.ProductSlug),
...
WHERE TABLE_SCHEMA = DATABASE()
  ...
  AND COLUMN_NAME = @SourceColumn;";
    ...
    {
        var p = Abbrev(row.ProductEnglishName, null, 2);
        var c = Abbrev(row.CategorySlug, null, 2);
        var b = Abbrev(row.BrandEnglishName, null, 2);
    ...
    private static class ProductRowMapper
    {
        public static ProductMappedRow Map(IReadOnlyDictionary<string, string> row)
        {
            ...
            var kgCash = ToToman(GetDecimal(row, "KgPriceCash", "SaleKgPriceCash", "cash_price", "Cash_Price", "cash_sale_price", "قیمت کیلو نقد", "قیمت نقد کیلویی", "قیمت فروش نقد", "فروش نقد", "قیمت نقد"));
            var kgCredit = ToToman(GetDecimal(row, "KgPriceInstallment", "SaleKgPriceInstallment", "credit_price", "Credit_Price", "installment_sale_price", "قیمت کیلو شرایطی", "قیمت شرایطی کیلویی", "قیمت فروش شرایطی", "فروش شرایطی"));
            var buyKgCash = ToToman(GetDecimal(row, "PurchaseKgPriceCash", "BuyKgPriceCash", "purchase_cash_price", "قیمت خرید نقد", "خرید نقد", "قیمت خرید نقد کیلویی"));
            var buyKgCredit = ToToman(GetDecimal(row, "PurchaseKgPriceInstallment", "BuyKgPriceInstallment", "purchase_credit_price", "قیمت خرید شرایطی", "خرید شرایطی", "قیمت خرید شرایطی کیلویی"));
            var packagingFee = ToToman(GetDecimal(row, "PackagingPricePerPack", "PackageFee", "PackagingFee", "Package_Price", "قیمت بسته بندی", "هزینه بسته بندی", "قیمت بسته‌بندی"));
