  ...
  AND COLUMN_NAME = @SourceColumn;";
    ...
    private static class ProductRowMapper
    {
        public static ProductMappedRow Map(IReadOnlyDictionary<string, string> row)
        {
            ...
            var category = Get(row, "CategoryName", "Category", "Base_terms", "BaseTerms", "terms", "En_Taxonomic", "Taxonomic", "دسته", "دسته بندی", "دسته‌بندی", "گروه اصلی");
            ...
            var saleCash = GetDecimal(row, "SalePriceCash", "CashSalePrice", "cash_price", "Cash_Price", "cash_sale_price", "regular_price", "RegularPrice", "قیمت فروش نقد", "فروش نقد", "قیمت نقد");
            var saleInstallment = GetDecimal(row, "SalePriceInstallment", "InstallmentSalePrice", "credit_price", "Credit_Price", "installment_sale_price", "شرایطی", "قیمت فروش شرایطی", "فروش شرایطی");
            var buyCash = GetDecimal(row, "PurchasePriceCash", "CashPurchasePrice", "purchase_cash_price", "قیمت خرید نقد", "خرید نقد");
            var buyInstallment = GetDecimal(row, "PurchasePriceInstallment", "InstallmentPurchasePrice", "purchase_credit_price", "قیمت خرید شرایطی", "خرید شرایطی");
            ...
            return new ProductMappedRow(
                ...
                ProductSlug: slug,
                Sku: sku,
                BrandName: Get(row, "Brand", "BrandName", "Brand_En", "برند"),
                PackageName: package,
                UnitWeight: GetDecimal(row, "UnitWeight", "Weight", "Pakage_One_kg", "Package_One_kg", "PackageOneKg", "وزن", "وزن واحد"),
                PacksPerCarton: GetInt(row, "PacksPerCarton", "QuantityInBox", "CartonQuantity", "تعداد در کارتن", "تعداد کارتن") ?? 1,
                CartonQuantity: GetInt(row, "CartonQuantity", "QuantityInBox", "PacksPerCarton", "تعداد در کارتن", "تعداد داخل کارتن") ?? 1,
